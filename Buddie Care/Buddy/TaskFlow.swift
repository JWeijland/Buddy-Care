...
struct TaskInProgressView: View {
    ...
    @State private var showQRCheckIn = false
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showQRCheckIn) {
                QRCheckInView {
                    showQRCheckIn = false
                    appState.buddyArrives()
                    stage = .inProgress
                }
            }
        }
        ...
        VStack(spacing: BCSpacing.md) {
            ...
            BCCard {
                VStack(spacing: BCSpacing.md) {
                    ...
                    Text("U staat voor de deur")
                        .font(BCTypography.title3)
                        .foregroundStyle(BCColors.textPrimary)
                    Text("Scan de QR-code op de Buddy Care doorbelcamera om officieel in te checken.")
                        .font(BCTypography.body)
                        .foregroundStyle(BCColors.textSecondary)
    ...
    private var bottomBar: some View {
        VStack {
            ...
            case .onTheWay:
                ...
                .padding(BCSpacing.lg)
            case .atDoor:
                BCPrimaryButton(title: "Scan QR-code", icon: "qrcode.viewfinder") {
                    showQRCheckIn = true
                }
                .padding(BCSpacing.lg)
            case .inProgress:
...
// MARK: - QR Check-in view (mock)

private struct QRCheckInView: View {
    let onDone: () -> Void
    @Environment(\.dismiss) private var dismiss
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: BCSpacing.lg) {
                ...
                Text("(Prototype: druk op 'Simuleer scan' om verder te gaan)")
                    .font(BCTypography.caption)
                    .foregroundStyle(.white.opacity(0.7))
                Spacer()
                BCPrimaryButton(title: "Simuleer scan", icon: "checkmark") {
                    onDone()
                }
                .padding(.horizontal, BCSpacing.lg)
